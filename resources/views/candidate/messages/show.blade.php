        ...
        <div class="row justify-content-center">
            <div class="col-12 col-lg-8">
                <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
                    <div>
                        <h1 class="panel-title">{{ $conversation->employer?->company_name ?? 'Employer' }}</h1>
                        <p class="panel-subtitle">{{ $conversation->displaySubject() }}</p>
                    </div>
                    <a href="{{ route('candidate.messages.index') }}" class="btn btn-outline-secondary">Inbox</a>
                </div>

                <div class="panel-card mb-4">
                    <div class="panel-card-body"
                         data-message-thread
                         data-updates-url="{{ route('candidate.messages.updates', $conversation) }}"
                         data-last-id="{{ $lastMessageId }}"
                         style="max-height:420px; overflow:auto">
                        @forelse ($messages as $message)
                            @php($mine = $message->isFromCandidate())
                ...
                <div class="panel-card">
                    <div class="panel-card-body">
                        <form method="POST"
                              action="{{ route('candidate.messages.store', $conversation) }}"
                              data-message-form
                              data-send-url="{{ route('candidate.messages.store', $conversation) }}">
                            @csrf
                            <label for="body" class="form-label required">Reply</label>
