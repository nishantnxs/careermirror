...
class GoogleAuthController extends Controller
{
    ...
    public function redirect(): RedirectResponse
    {
        ...
        return redirect()->away(
            $this->google->authorizationUrl(route('employer.auth.google.callback'))
        );
    }
    ...
    public function callback(Request $request): RedirectResponse
    {
        try {
            $profile = $this->google->userFromCallback(
                $request,
                route('employer.auth.google.callback'),
            );
            $result = $this->google->resolveEmployer($profile);
        } catch (ValidationException $exception) {
            return redirect()->route('employer.login')->withErrors($exception->errors());
        } catch (\Throwable) {
            return redirect()->route('employer.login')
                ->withErrors(['email' => 'Google sign-in could not be completed. Please try again.']);
        }

        Auth::guard('employer')->login($result['account']);
        ...
        return redirect()->intended(route('employer.dashboard'))->with('success', $message);
    }
}
